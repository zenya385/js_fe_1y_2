...
const acordionBtnEl = document.querySelectorAll(".accordion-list-item");
const handleAcordBtnClick = (e) => {
  const acordBtn = e.target;
  const answerPanel = acordBtn.nextElementSibling;
  answerPanel.classList.toggle("active");
};
acordionBtnEl.forEach((acordBtn) => {
  acordBtn.addEventListener("click", handleAcordBtnClick);
});
